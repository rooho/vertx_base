package com.scaffolding.core.handlerfactory;
...
public class RouterHandlerFactory {
    ...
    public Router createRouter(Handler<RoutingContext> ...var) {
        ...
        /* 添加跨域的方法 **/
        router.route().handler(CorsHandler.create().addOrigin("*").allowedMethods(method));
        ...
        try {
            ...
            for (Class<?> handler : sortedHandlers) {
                try {
                    ...
                    LOGGER.error("Error register {}, detail: {}", handler, e.getMessage());
                }
            }
        } catch (Exception e) {
